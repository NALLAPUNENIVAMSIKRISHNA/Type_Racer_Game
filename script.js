...
function handleTyping() {
  ...
  if (typedTextWithSpaces.length > 0 && typedTextWithSpaces[typedTextWithSpaces.length - 1] === ' ') {
    typedSpacesCount--; // If the last character in typedTextWithSpaces is a space, reduce the count by 1
  }

  // Subtract the spaces between words from the typedText without extra spaces length
  const typedWordsLength = typedTextWithoutExtraSpaces.split(' ').length;
  const expectedTypedLength = typedTextWithoutExtraSpaces.length + typedSpacesCount - (typedWordsLength - 1);

  if (expectedTypedLength === actualQuote.replace(/\s/g, '').length) {
    cancelAnimationFrame(timerId);
    quoteInputElement.setAttribute('disabled', 'disabled');
    submitButton.style.display = 'block';
  }
